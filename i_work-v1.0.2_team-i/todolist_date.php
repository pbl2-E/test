...
function Date_Sort(){
    ...
    for($i = 0; $i < count($item) - 1; $i++){
        list($date, $body, $pri) = explode(",", $item[$i], 3);  //itemを日付と内容、優先度に分割する
        list($year, $month, $day) = explode("/", $date, 3);    //dateを年と月、日で分割する
        $com[$i] = (int)$year*365 + (int)$month*31 + (int)$day;   //比較用の配列にそのitemの日付の大きさを代入

    }

    for($i=0; $i<count($item)-1; $i++){   //itemの中身を昇順に並び替え
    ...
    for($i = 0; $i < count($item) - 1; $i++){
        list($date0[$i], $body0[$i], $pri0[$i]) = explode(",", $item[$i], 3);  //itemを日付と内容、優先度に分割する
    }


for($i = 0; $i < count($item)-1; $i++){
        echo "<p>・$date0[$i]&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;$body0[$i]<br></p>";
    }
}
