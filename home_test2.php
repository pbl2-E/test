...
<body>
  ...
  <font >ようこそ</font>
  <div class="explain">
<input type="button" value="アプリ操作説明へ" onClick="location.href='explain.html'"><br>
  </div>
  <?php session_start();
 function h($s){
...
$f = file_get_contents("file_operator.txt");
        ...
        for($i = 0; $i < count($item) - 1; $i++){
          ...
          if($id[$i] == $ID){
            ...
            echo (
              ...
              "<a href='http://sshg.cs.ehime-u.ac.jp/~g475yama/pbl2/ContentsPage2.php?file_name=$item[$i]'>");
            echo h($item_name);
            echo("</a>");
            echo "</td>";
 ...
 if($ach == null){
              echo "<td>";
              echo ("<a href=https://search.yahoo.co.jp/search?p=$task_name target=_blank>調べちゃおう</a>");
              echo "</td>";
            }
            if($id[$i] == $ID){
              echo "<td>";
